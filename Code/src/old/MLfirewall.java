...
public class MLfirewall {
    ...
    private void processPacket(PacketContext context) {
		...
		if(SERVER_DECISION_ALLOW) {
			log.warn("{} : {} ({}) to {} ({}): {}", MSG_ALLOWING, srcIPv4, srcMAC, dstIPv4, dstMAC, deviceId);
			allowPackets(deviceId, srcMAC, srcIPv4, srcPort, dstMAC, dstIPv4, dstPort);
//			context.send();
		} else {
			log.warn("{} : {} ({}) to {} ({}): {}", MSG_BLOCKING, srcIPv4, srcMAC, dstIPv4, dstMAC, deviceId);
			banPackets(deviceId, srcIPv4, srcMAC, dstIPv4, dstMAC, srcPort, dstPort);
			context.block();
		}
    ...
    private void banPackets(DeviceId deviceId, MacAddress srcMAC, IpAddress srcIPv4, int srcPort, MacAddress dstMAC, IpAddress dstIPv4, int dstPort) {
		// initialize TrafficSelector
		TrafficSelector selector;
		
		// Don't use srcPort for now. dstPort is more important.
		if (dstPort == 0) {
			// Ignore port.
    ...
    private void allowPackets(DeviceId deviceId, MacAddress srcMAC, MacAddress dstMAC) {
