...
from pyspark.sql.functions import stddev, max, mean
import numpy as np
...
def calcDistance(point, clusterCenter, clusterCenters):
	...
	for c in clusterCenters:
		...
		# If both cluster and point are > 0 and point < cluster, then set equal.
		# If both cluster and point are < 0 and point > cluster, then set equal.
		if ((c[0] > 0 and point_x > 0) and (point_x < c[0])) or ((c[0] < 0 and point_x < 0) and (point_x > c[0])):
			x_safe = True
		if ((c[1] > 0 and point_y > 0) and (point_y < c[1])) or ((c[1] < 0 and point_y < 0) and (point_y > c[1])):
			y_safe = True
		if ((c[2] > 0 and point_z > 0) and (point_z < c[2])) or ((c[2] < 0 and point_z < 0) and (point_z > c[2])):
			z_safe = True
...
def checkAnomaly(data):
	...
	for row in data:
		...
		for i in range(0, 3):
			...
			# # # distFromCenter, actualDistFromCenter = calcDistance(point, center, clusterCenters)
			...
			# # # if (minDistance is None) or (minDistance > actualDistFromCenter):
				# # # minDistance = actualDistFromCenter
...
def readDataFromES():
	...
	sumOfBytes_0_stddev = df_pred.filter(df_pred['prediction'] == 0).select(stddev('sumOfBytes')).collect()[0][0]
	uniqDstIPs_0_stddev = df_pred.filter(df_pred['prediction'] == 0).select(stddev('uniqDstIPs')).collect()[0][0]
	uniqDstPorts_0_stddev = df_pred.filter(df_pred['prediction'] == 0).select(stddev('uniqDstPorts')).collect()[0][0]
	sumOfBytes_1_stddev = df_pred.filter(df_pred['prediction'] == 1).select(stddev('sumOfBytes')).collect()[0][0]
	uniqDstIPs_1_stddev = df_pred.filter(df_pred['prediction'] == 1).select(stddev('uniqDstIPs')).collect()[0][0]
	uniqDstPorts_1_stddev = df_pred.filter(df_pred['prediction'] == 1).select(stddev('uniqDstPorts')).collect()[0][0]
	sumOfBytes_2_stddev = df_pred.filter(df_pred['prediction'] == 2).select(stddev('sumOfBytes')).collect()[0][0]
	uniqDstIPs_2_stddev = df_pred.filter(df_pred['prediction'] == 2).select(stddev('uniqDstIPs')).collect()[0][0]
	uniqDstPorts_2_stddev = df_pred.filter(df_pred['prediction'] == 2).select(stddev('uniqDstPorts')).collect()[0][0]
	
	printDebugMsg("sumOfBytes_0_stddev = {0}".format(sumOfBytes_0_stddev))
	printDebugMsg("uniqDstIPs_0_stddev = {0}".format(uniqDstIPs_0_stddev))
	printDebugMsg("uniqDstPorts_0_stddev = {0}".format(uniqDstPorts_0_stddev))
	printDebugMsg("sumOfBytes_1_stddev = {0}".format(sumOfBytes_1_stddev))
	printDebugMsg("uniqDstIPs_1_stddev = {0}".format(uniqDstIPs_1_stddev))
	printDebugMsg("uniqDstPorts_1_stddev = {0}".format(uniqDstPorts_1_stddev))
	printDebugMsg("sumOfBytes_2_stddev = {0}".format(sumOfBytes_2_stddev))
	printDebugMsg("uniqDstIPs_2_stddev = {0}".format(uniqDstIPs_2_stddev))
	printDebugMsg("uniqDstPorts_2_stddev = {0}".format(uniqDstPorts_2_stddev))
	
	sumOfBytes_0_mean = df_pred.filter(df_pred['prediction'] == 0).select(mean('sumOfBytes')).collect()[0][0]
	uniqDstIPs_0_mean = df_pred.filter(df_pred['prediction'] == 0).select(mean('uniqDstIPs')).collect()[0][0]
	uniqDstPorts_0_mean = df_pred.filter(df_pred['prediction'] == 0).select(mean('uniqDstPorts')).collect()[0][0]
	sumOfBytes_1_mean = df_pred.filter(df_pred['prediction'] == 1).select(mean('sumOfBytes')).collect()[0][0]
	uniqDstIPs_1_mean = df_pred.filter(df_pred['prediction'] == 1).select(mean('uniqDstIPs')).collect()[0][0]
	uniqDstPorts_1_mean = df_pred.filter(df_pred['prediction'] == 1).select(mean('uniqDstPorts')).collect()[0][0]
	sumOfBytes_2_mean = df_pred.filter(df_pred['prediction'] == 2).select(mean('sumOfBytes')).collect()[0][0]
	uniqDstIPs_2_mean = df_pred.filter(df_pred['prediction'] == 2).select(mean('uniqDstIPs')).collect()[0][0]
	uniqDstPorts_2_mean = df_pred.filter(df_pred['prediction'] == 2).select(mean('uniqDstPorts')).collect()[0][0]
	
	printDebugMsg("sumOfBytes_0_mean = {0}".format(sumOfBytes_0_mean))
	printDebugMsg("uniqDstIPs_0_mean = {0}".format(uniqDstIPs_0_mean))
	printDebugMsg("uniqDstPorts_0_mean = {0}".format(uniqDstPorts_0_mean))
	printDebugMsg("sumOfBytes_1_mean = {0}".format(sumOfBytes_1_mean))
	printDebugMsg("uniqDstIPs_1_mean = {0}".format(uniqDstIPs_1_mean))
	printDebugMsg("uniqDstPorts_1_mean = {0}".format(uniqDstPorts_1_mean))
	printDebugMsg("sumOfBytes_2_mean = {0}".format(sumOfBytes_2_mean))
	printDebugMsg("uniqDstIPs_2_mean = {0}".format(uniqDstIPs_2_mean))
	printDebugMsg("uniqDstPorts_2_mean = {0}".format(uniqDstPorts_2_mean))
	
	numStdDevAboveMean_0_Bytes = 2
	while (sumOfBytes_0_max > sumOfBytes_0_mean + (numStdDevAboveMean_0_Bytes * sumOfBytes_0_stddev)):
		# # # printDebugMsg("Increasing numStdDevAboveMean_0_Bytes")
		numStdDevAboveMean_0_Bytes += 1
	upperThreshold_0_Bytes = int(ceil(sumOfBytes_0_mean + numStdDevAboveMean_0_Bytes * sumOfBytes_0_stddev))
	printDebugMsg("numStdDevAboveMean_0_Bytes = {0}".format(numStdDevAboveMean_0_Bytes))
	printDebugMsg("upperThreshold_0_Bytes = {0}".format(upperThreshold_0_Bytes))
	
	numStdDevAboveMean_1_Bytes = 2
	while (sumOfBytes_1_max > sumOfBytes_1_mean + (numStdDevAboveMean_1_Bytes * sumOfBytes_1_stddev)):
		# # # printDebugMsg("Increasing numStdDevAboveMean_1_Bytes")
		numStdDevAboveMean_1_Bytes += 1
	upperThreshold_1_Bytes = int(ceil(sumOfBytes_1_mean + numStdDevAboveMean_1_Bytes * sumOfBytes_1_stddev))
	printDebugMsg("numStdDevAboveMean_1_Bytes = {0}".format(numStdDevAboveMean_1_Bytes))
	printDebugMsg("upperThreshold_1_Bytes = {0}".format(upperThreshold_1_Bytes))
	
	numStdDevAboveMean_2_Bytes = 2
	while (sumOfBytes_2_max > sumOfBytes_2_mean + (numStdDevAboveMean_2_Bytes * sumOfBytes_2_stddev)):
		# # # printDebugMsg("Increasing numStdDevAboveMean_2_Bytes")
		numStdDevAboveMean_2_Bytes += 1
	upperThreshold_2_Bytes = int(ceil(sumOfBytes_2_mean + numStdDevAboveMean_2_Bytes * sumOfBytes_2_stddev))
	printDebugMsg("numStdDevAboveMean_2_Bytes = {0}".format(numStdDevAboveMean_2_Bytes))
	printDebugMsg("upperThreshold_2_Bytes = {0}".format(upperThreshold_2_Bytes))
	
	numStdDevAboveMean_0_DstIPs = 2
	while (uniqDstIPs_0_max > uniqDstIPs_0_mean + (numStdDevAboveMean_0_DstIPs * uniqDstIPs_0_stddev)):
		# # # printDebugMsg("Increasing numStdDevAboveMean_0_DstIPs")
		numStdDevAboveMean_0_DstIPs += 1
	upperThreshold_0_DstIPs = int(ceil(uniqDstIPs_0_mean + numStdDevAboveMean_0_DstIPs * uniqDstIPs_0_stddev))
	printDebugMsg("numStdDevAboveMean_0_DstIPs = {0}".format(numStdDevAboveMean_0_DstIPs))
	printDebugMsg("upperThreshold_0_DstIPs = {0}".format(upperThreshold_0_DstIPs))
	
	numStdDevAboveMean_1_DstIPs = 2
	while (uniqDstIPs_1_max > uniqDstIPs_1_mean + (numStdDevAboveMean_1_DstIPs * uniqDstIPs_1_stddev)):
		# # # printDebugMsg("Increasing numStdDevAboveMean_1_DstIPs")
		numStdDevAboveMean_1_DstIPs += 1
	upperThreshold_1_DstIPs = int(ceil(uniqDstIPs_1_mean + numStdDevAboveMean_1_DstIPs * uniqDstIPs_1_stddev))
	printDebugMsg("numStdDevAboveMean_1_DstIPs = {0}".format(numStdDevAboveMean_1_DstIPs))
	printDebugMsg("upperThreshold_1_DstIPs = {0}".format(upperThreshold_1_DstIPs))
	
	numStdDevAboveMean_2_DstIPs = 2
	while (uniqDstIPs_2_max > uniqDstIPs_2_mean + (numStdDevAboveMean_2_DstIPs * uniqDstIPs_2_stddev)):
		# # # printDebugMsg("Increasing numStdDevAboveMean_2_DstIPs")
		numStdDevAboveMean_2_DstIPs += 1
	upperThreshold_2_DstIPs = int(ceil(uniqDstIPs_2_mean + numStdDevAboveMean_2_DstIPs * uniqDstIPs_2_stddev))
	printDebugMsg("numStdDevAboveMean_2_DstIPs = {0}".format(numStdDevAboveMean_2_DstIPs))
	printDebugMsg("upperThreshold_2_DstIPs = {0}".format(upperThreshold_2_DstIPs))
	
	numStdDevAboveMean_0_DstPorts = 2
	while (uniqDstPorts_0_max > uniqDstPorts_0_mean + (numStdDevAboveMean_0_DstPorts * uniqDstPorts_0_stddev)):
		# # # printDebugMsg("Increasing numStdDevAboveMean_0_DstPorts")
		numStdDevAboveMean_0_DstPorts += 1
	upperThreshold_0_DstPorts = int(ceil(uniqDstPorts_0_mean + numStdDevAboveMean_0_DstPorts * uniqDstPorts_0_stddev))
	printDebugMsg("numStdDevAboveMean_0_DstPorts = {0}".format(numStdDevAboveMean_0_DstPorts))
	printDebugMsg("upperThreshold_0_DstPorts = {0}".format(upperThreshold_0_DstPorts))
	
	numStdDevAboveMean_1_DstPorts = 2
	while (uniqDstPorts_1_max > uniqDstPorts_1_mean + (numStdDevAboveMean_1_DstPorts * uniqDstPorts_1_stddev)):
		# # # printDebugMsg("Increasing numStdDevAboveMean_1_DstPorts")
		numStdDevAboveMean_1_DstPorts += 1
	upperThreshold_1_DstPorts = int(ceil(uniqDstPorts_1_mean + numStdDevAboveMean_1_DstPorts * uniqDstPorts_1_stddev))
	printDebugMsg("numStdDevAboveMean_1_DstPorts = {0}".format(numStdDevAboveMean_1_DstPorts))
	printDebugMsg("upperThreshold_1_DstPorts = {0}".format(upperThreshold_1_DstPorts))
	
	numStdDevAboveMean_2_DstPorts = 2
	while (uniqDstPorts_2_max > uniqDstPorts_2_mean + (numStdDevAboveMean_2_DstPorts * uniqDstPorts_2_stddev)):
		# # # printDebugMsg("Increasing numStdDevAboveMean_2_DstPorts")
		numStdDevAboveMean_2_DstPorts += 1
	upperThreshold_2_DstPorts = int(ceil(uniqDstPorts_2_mean + numStdDevAboveMean_2_DstPorts * uniqDstPorts_2_stddev))
	printDebugMsg("numStdDevAboveMean_2_DstPorts = {0}".format(numStdDevAboveMean_2_DstPorts))
	...
	sumOfBytes_0 = (sumOfBytes_0_max, sumOfBytes_0_mean, sumOfBytes_0_stddev)
	sumOfBytes_1 = (sumOfBytes_1_max, sumOfBytes_1_mean, sumOfBytes_1_stddev)
	sumOfBytes_2 = (sumOfBytes_2_max, sumOfBytes_2_mean, sumOfBytes_2_stddev)
	uniqDstIPs_0 = (uniqDstIPs_0_max, uniqDstIPs_0_mean, uniqDstIPs_0_stddev)
	uniqDstIPs_1 = (uniqDstIPs_1_max, uniqDstIPs_1_mean, uniqDstIPs_1_stddev)
	uniqDstIPs_2 = (uniqDstIPs_2_max, uniqDstIPs_2_mean, uniqDstIPs_2_stddev)
	uniqDstPorts_0 = (uniqDstPorts_0_max, uniqDstPorts_0_mean, uniqDstPorts_0_stddev)
	uniqDstPorts_1 = (uniqDstPorts_1_max, uniqDstPorts_1_mean, uniqDstPorts_1_stddev)
	uniqDstPorts_2 = (uniqDstPorts_2_max, uniqDstPorts_2_mean, uniqDstPorts_2_stddev)
